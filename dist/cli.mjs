...
import { d as parseIntegerInput, f as parseOutputDetailInput, i as buildMarkdownSummary, l as getLogger, p as _enum, t as runLinkCheck } from "./run-link-check-Be7bi0w3.mjs";
...
function createProgram() {
	const program = new Command().name("link-checker").description("Check markdown links locally or in CI. Run `link-checker scan --help` or `link-checker scan -h` to see scan options.");
	program.command("scan").description("Scan repository files for links.").option("-c, --config <path>", "Path to a JSON or JSONC config file").option("--allow-domain <domain...>", "Domains that may downgrade failures to warnings").option("--allow-status-code <code...>", "HTTP status codes to treat as OK").option("-p, --paths <glob...>", "Markdown paths or globs to scan").option("--exclude <glob...>", "Paths or globs to exclude").option("--deny-domain <domain...>", "Domains that upgrade warnings to errors").option("--deny-status-code <code...>", "HTTP status codes to force as errors").option("--timeout-seconds <number>", "Per-request timeout in seconds", parseInteger).option("--concurrency <number>", "Maximum concurrent URL checks", parseInteger).option("--retry-base-delay-ms <number>", "Base delay between retries in milliseconds", parseInteger).option("--retry-max-delay-ms <number>", "Maximum delay between retries in milliseconds", parseInteger).option("--retries <number>", "Retry attempts for retryable failures", parseInteger).option("--fail-on <mode>", "Failure mode: error, warning, or mixed").option("--detail <level>", "Issue detail level: simple or detailed", parseOutputDetail).option("--detailed", "Alias for --detail detailed").option("--ignore-url-pattern <pattern...>", "URL patterns to skip").option("--ignore-domain <domain...>", "Domains to skip").option("--user-agent <value>", "Override the default user agent").option("--output-json <path>", "Write a JSON report to this path").option("--format <type>", "Output format: text, json, or markdown", parseOutputFormat, "text").option("--issues-only", "Only include errors and warnings in rendered output").option("--verbose", "Emit verbose output").option("--no-respect-gitignore", "Do not respect .gitignore during discovery").action(async (options) => {
		const format = options.format;
		const outputDetail = options.detail ?? (options.detailed ? "detailed" : void 0);
		const result = await runWithRequestedFormat(format, () => runLinkCheck({
...
function renderResult(result, format, options = {}) {
	const renderedResult = options.issuesOnly ? filterResultToIssues(result) : result;
	if (format === "json") return JSON.stringify(renderedResult, null, 2);
	return buildMarkdownSummary(renderedResult);
}
function filterResultToIssues(result) {
	return {
		...result,
...
async function runWithRequestedFormat(format, action) {
	if (format !== "json") return action();
	const previousLogLevel = process.env.LOG_LEVEL;
	process.env.LOG_LEVEL = "silent";
	try {
		return await action();
	} finally {
		if (previousLogLevel === void 0) delete process.env.LOG_LEVEL;
...
function parseOutputFormat(value) {
	...
	const result = _enum([
		...
		"json",
		"markdown"
	], { error: errorMessage }).safeParse(value);
	if (result.success) return result.data;
	throw new Error(errorMessage);
}
function parseOutputDetail(value) {
	return parseOutputDetailInput(value, `Expected issue detail level to be simple or detailed. Received "${value}".`);
}
...
function writeTextResult(result) {
	...
	if (result.reportPath) logger.info({ reportPath: result.reportPath }, "Wrote report file");
}
if (isDirectExecution()) await runCli();
//#endregion
export { createProgram, renderResult, runCli };
